...
import 'package:morse_comms/features/lessons/ui/koch_lessons_screen.dart';
...
class _FakeLessonRepository implements LessonRepository {
  ...
  @override
  double? bestAccuracy(int level) => level == 2 ? 0.85 : null;

  @override
  Map<int, double> loadAllBestAccuracy() => {2: 0.85};
...
void main() {
  testWidgets('KochLessonsScreen shows Koch Method title',
    ...
    await tester.pumpWidget(
      _buildKochLessonsApp(repo: repo, player: player, settings: settings),
    );

    expect(find.text('Koch Method'), findsOneWidget);
  });

  testWidgets('KochLessonsScreen shows progress header',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
    ...
    // Progress header should show current level info
    expect(find.byIcon(Icons.school), findsOneWidget);
    expect(find.textContaining('Level'), findsAtLeastNWidgets(1));
  });

  testWidgets('KochLessonsScreen shows lesson tiles',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
    ...
    // Should show lesson tiles (at least the unlocked ones)
    expect(find.byType(ListTile), findsAtLeastNWidgets(1));
  });

  testWidgets('KochLessonsScreen shows accuracy chip for completed level',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
  ...
  testWidgets('KochLessonsScreen shows locked icon for locked levels',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
  ...
  testWidgets('KochLessonsScreen opens reference screen from app bar',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
  ...
  testWidgets('KochLessonsScreen opens lessons info dialog',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
    ...
    // Info dialog should appear with Koch content
    expect(find.textContaining('Koch'), findsAtLeastNWidgets(1));
  });

  testWidgets('KochLessonsScreen navigates to drill when tapping unlocked level',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
  ...
  testWidgets('KochLessonsScreen does not navigate when tapping locked level',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
    ...
    if (lockIcons.evaluate().isNotEmpty) {
      ...
      // Should still be on Koch lessons screen
      expect(find.text('Koch Method'), findsOneWidget);
    }
  });

  testWidgets('KochLessonsScreen shows best accuracy in progress header',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
  ...
  testWidgets('KochLessonsScreen shows active characters in progress header',
      (WidgetTester tester) async {
    final repo = _FakeLessonRepository();
    final player = _StubPlayerService();
    final settings = await _makeSettingsCubit();
    ...
    // Should show active characters text
    expect(find.textContaining('Active characters'), findsOneWidget);
  });
}
