...
class OfflineAnalyzer {
  ...
  static const double _minRatio = 2.0;
  ...
  static const double _maxClusterCv = 0.45;
  ...
  static String analyze(List<double> magnitudes, double frameDurationMs) {
    ...
    // Noise filter: ignore ON events shorter than 2.5 × frameDuration.
    ...
    // NOTE: At 30–40 WPM, dots are 2–3 frames. We deliberately use 2.5×
    // (not 3×) to avoid filtering genuine short dots at high WPM.
  ...
  static String _analyzeSegment(
    ...
    // For the seeded path dotMs is known, so tighten the transient filter to
    // 70% of a dot.  This catches clicks whose apparent duration (due to a
    // partial Goertzel frame staying above threshold) exceeds minOnMs but is
    // still clearly below any genuine Morse element.  At low WPM a 50ms click
    // gives ~58ms ON; at 15 WPM dotMs≈93ms → threshold = max(29ms, 65ms) = 65ms.
    // At 40 WPM dotMs≈35ms → threshold = max(29ms, 24ms) = 29ms (unchanged).
    final seededSeg = _filterShortOns(trimmed, max(minOnMs, dotMs * 0.7));
  ...
  ///
  /// Scans for the **first** relative jump ≥ 2.0 between consecutive sorted
  ...
  static (double, double, bool) _robustBimodalSplit(List<double> onDurs) {
    ...
    for (int split = 1; split < s.length; split++) {
      ...
      if (_cv(lower) > _maxClusterCv) continue;
      if (_cv(upper) > _maxClusterCv) continue;
    ...
    return (
      _median(s.sublist(0, bestSplit)),
      _median(s.sublist(bestSplit)),
      true,
    );
  }
  ...
  static List<int> _detectSpeedChanges(List<(bool, double)> events) {
    ...
    // If bimodal split works well with low CV, no speed changes detected
    if (fullValid) {
      ...
      // Tightened threshold: only split if clusters have HIGH variance
      if (cvLower < 0.45 && cvUpper < 0.45) {
        return [];
      }
    }
    ...
    const minSegmentSize = 6; // Minimum ON events per sub-segment
    ...
    for (int i = minSegmentSize; i < onEvents.length - minSegmentSize; i++) {
      ...
      if (beforeValid && afterValid) {
        ...
        // Require >50% speed difference to avoid false positives
        if (speedRatio > 1.5) {
          // Verify both sub-segments have consistent timing (low CV)
          ...
          if (beforeCvLower < 0.35 && beforeCvUpper < 0.35 &&
              afterCvLower < 0.35 && afterCvUpper < 0.35) {
